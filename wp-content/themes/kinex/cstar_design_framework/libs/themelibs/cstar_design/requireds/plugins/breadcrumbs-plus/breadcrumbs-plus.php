 ...
 * @since 0.1
 */
/*load_plugin_textdomain( T_NAME, false, 'breadcrumbs-plus/languages' );*/
...
function breadcrumbs_plus_get_items( $args ) {
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' === $wp_query->post->post_type ) {
         ...
         $post_options        = get_post_meta($post_id, '_post_options', true);
         $parent_page_id      = @$post_options["_post_breadcrumbs_page"];
         
         if( empty($parent_page_id) ){
            
            if( is_single_portfolio() ){
              $parent_page_id = get_theme_option('portfolio','portfolio_page');
            } else if( is_single() ){
              $parent_page_id = get_theme_option('blog','blog_page');
            }
         } 

         $parents = breadcrumbs_plus_get_parents( $parent_page_id );
         $item	= array_merge( $item, $parents );         
         
		} else if ( 'page' !== $wp_query->post->post_type ) {
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			if ( is_day() )
				$item['last'] = __( 'Archives for ', T_NAME ) . get_the_time( 'F j, Y' );

			elseif ( is_month() )
				$item['last'] = __( 'Archives for ', T_NAME ) . single_month_title( ' ', false );

			elseif ( is_year() )
				$item['last'] = __( 'Archives for ', T_NAME ) . get_the_time( 'Y' );
		}

		else if ( is_author() )
			$item['last'] = __( 'Archives by: ', T_NAME ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
	}

	/* If viewing search results. */
	else if ( is_search() )
		$item['last'] = __( 'Search results for "', T_NAME ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

	/* If viewing a 404 error page. */
	else if ( is_404() )
		$item['last'] = __( 'Page Not Found', T_NAME );
...
function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() )
			$item[] = __( 'Split', 'breadcrumbs' );

		elseif ( bbp_is_topic_merge() )
			$item[] = __( 'Merge', 'breadcrumbs' );

		elseif ( bbp_is_topic_edit() )
			$item[] = __( 'Edit', 'breadcrumbs' );
	}
	...
	elseif ( bbp_is_single_forum() ) {
		...
		$forum_parent_id = bbp_get_forum_parent_id();
