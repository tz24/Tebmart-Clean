...
add_shortcode( 'advancedsearch', 'advanced_search' );
	...
	if($edition != NULL or $author != NULL or $ccode != NULL or $search != NULL){
			...
			/*if($edition != NULL){ $edition_atr = "('pa_edition')"; }	
			if($author != NULL){ $author_atr = "('pa_author-name')"; }
			if($ccode != NULL){ $ccode_atr = "('pa_course-code')"; }
			if($search != NULL){ $search_atr = "('pa_edition'),('pa_author-name'),('pa_course-code')"; }
			*/
			...
			$all_rec_q = "SELECT term_taxonomy_id,term_id FROM ".$wpdb->prefix."term_taxonomy".$where4 ;
			$hswp_tables = $wpdb->get_results($all_rec_q);
			...
			if($edition != NULL or $search != NULL){ $edition_value = "AND name LIKE '%".$edition.$search."%' "; }	
			if($author != NULL or $search != NULL){ $author_value = "AND slug LIKE '%".$author.$search."%' "; }
			...
			$where6 = " WHERE term_taxonomy_id IN(".$new4.") ".$isbn_value2."  ORDER BY object_id ASC";
		...
		if($total==0){
			/*echo do_shortcode('[alertbox color="red"]No results found for this search.[/alertbox]');
			echo "<br/><h4>Can't find the book you are looking for? We can help you find it.</h4>";*/
			...
			<div class="book_information" >
				...
				<div  class="book_metadata">
					...
					<p>
						...
						<span class="content"><?php print $product->get_sku(); ?></span>
					</p>
					<?php */ ?>
